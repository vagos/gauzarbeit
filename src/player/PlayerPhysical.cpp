...
void PlayerPhysical::moveDirection(std::shared_ptr<Thing> owner, World& world,
                                   const std::string& direction)
{

    if (direction == "left")
    {
        doMove(owner, world, current_room->x - 1, current_room->y);
    }
    ...
    {
        doMove(owner, world, current_room->x, current_room->y + 1);
    }
...
void PlayerPhysical::doMove(std::shared_ptr<Thing> owner, World& world, int x, int y)
{
    if (current_room)
        current_room->removePlayer(owner);
